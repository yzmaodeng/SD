...
//		bean.reduce("D:\\opt\\", "/log/http_imgload2.jpg", 500, 500, true);// 压缩图片
		...
		bean.water("F:\\需求资料\\酱骨汤需求页面\\水印\\0021976ebb3c1543f48b06.jpg", "F:\\需求资料\\酱骨汤需求页面\\水印\\水印.png");// 绘制水印
//
//		bean.rotate("D:\\opt\\", "/log/http_imgload2.jpg", 90, null);// 旋转图片
//
//		bean.zoom("D:\\opt\\", "/log/http_imgload2.jpg", 2, null);// 放大或缩小图片
//
//		bean.fontImg("D:\\opt\\log\\http_imgload2.jpg", "中国", new Font(
//				"Atlantic Inline", Font.PLAIN, 18), Color.GREEN, 200, 100);// 把字符印到图片上
	}
	 ...
	 * @param response
	 * */
	public void bringCode(HttpServletRequest request,
		...
		try {
			...
			/** 如果是做网页验证码，请使用这两行代码 */
			// request.getSession().setAttribute("checkCode", randCode);//
			// 认证码保存在session中
	...
	public String cut(String base, String imgPath, int left, int top,
	 ...
	 * @return String
	 */
	public String reduce(String base, String imgPath, int width, int height,
			boolean percentage) {
		try {
...
//					encoder.encode(image);
	 ...
	 * @return
	 */
	public String rotate(String base, String imgPath, int angle,
			HttpServletResponse response) {
		try {
			if (angle % 90 == 0) {
				...
				int width = src.getWidth();
				int height = src.getHeight();
				BufferedImage image = new BufferedImage(height, width, src
						.getColorModel().getTransparency());
				Graphics2D graphics2d = image.createGraphics();
				if (angle < 0) {
					graphics2d.rotate(Math.toRadians(angle), width / 2,
							width / 2);
				} else {
					graphics2d.rotate(Math.toRadians(angle), height / 2,
							height / 2);
				}
	 ...
	 * @return
	 */
	public String zoom(String base, String imgPath, double ratio,
			HttpServletResponse response) {
		try {
	 ...
	 * @return
	 */
	public void fontImg(String imgPath, String text, Font font, Color color,
			int left, int top) {
		try {
			BufferedImage src = ImageIO.read(new File(imgPath));
			int width = src.getWidth();
...
//			encoder.encode(image);
	 ...
	 * @return Image
	 */
	public static ImageUtil getBean() {
		return bean;
	}
